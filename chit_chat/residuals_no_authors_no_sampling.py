...
class Lstm(Model):
    ...
    def _compute_lstm_matrix_parameters(self, idx):
        if idx == 0:
            ...
            input_dim = self._num_nodes[0] + self._embedding_size + self._dim_compressed * self._connection_visibility
        else:
            input_dim = self._num_nodes[idx-1] + \
                        self._num_nodes[idx] + \
                        self._dim_compressed * self._connection_visibility
    ...
    def _validation_graph(self):
        with tf.name_scope('validation'):
            ...
            # print('saved_sample_for_connections:', saved_sample_for_connections)
            ...
            for layer_idx in range(self._num_layers):
                saved_sample_from_connections.append(
                    tf.Variable(tf.zeros([1, self._connection_visibility * self._dim_compressed]),
                                trainable=False,
                                name='saved_sample_from_connections_%s' % layer_idx))
            # print('saved_sample_from_connections:', saved_sample_from_connections)
            ...
            # print('sample_from_connections:', sample_from_connections)
